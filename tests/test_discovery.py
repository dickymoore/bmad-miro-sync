...
class DiscoveryTests(unittest.TestCase):
    ...
    def test_discovery_skips_sections_that_sanitize_to_empty_and_reparents_children(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            (root / "_bmad-output/planning-artifacts").mkdir(parents=True)
            (root / ".bmad-miro.toml").write_text(LEGACY_CONFIG, encoding="utf-8")
            ...
            self.assertNotIn("_bmad-output/planning-artifacts/prd.md#scope", artifact_ids)
